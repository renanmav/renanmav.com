...
- **Onboarding flow**: _Talk about onboarding flow_. After some time, there was another effort to implement instant onboarding, reducing the amount of clicks from ~10 to nearly 2. {/* TODO: add recording here */}
- **Decentralized ID onboarding**: an IC5 eng from the identity team was on PTO, so I was assigned to build the screens for claiming your `.cb.id` username, which is a lightweight version of ENS with really cheap gas fees, so we offer this for free to users during onboarding. Coinbase recently launched [basenames](https://www.base.org/names) to the public, so I think that makes `.cb.id` kinda legacy. _Say hello to [renan.base.eth](https://www.base.org/names/renan.base.eth)!_ {/* TODO: add recording here */}
- **Terms of Service and Privacy Policy**: a legal requirement for users to acknowledge. Most boring thing ever to implement, but someone has to do it. And that someone was me. At first, I thought about not breaking the DRY principle by rendereing a [pre-existing TOS page](https://wallet.coinbase.com/terms-of-service) within a webview, but due security reasons, I had to keep a copy of the page in our app. _Don't freeze the app on startup._
- **A/B experiments to improve funding rates**: NFT incentives that were delivered once the funding was completed, CTAs to launch the buy flow on different parts of the app, etc.
