...
1. **[react-navigation](https://reactnavigation.org) scaffolding**: Coinbase Wallet navigation setup in `apps/rn`, including `TabNavigator` and `DrawerNavigator`, as well as basic deep linking setup. At that time, we were using react-navigation v4. It was later updated to v5 and then to v6.
1. **Common ESLint config within monorepo**: we first started with yarn workspaces at `wallet/wallet-mobile` repo and then the infra team switched to NX; as one of my first challenges, I pulled out the common config for linting across our packages and put it in a separate monorepo package `workspaces/libs/eslint-config`.
1. **Onboarding flow**: _Talk about onboarding flow_. After some time, there was another effort to implement instant onboarding, reducing the amount of clicks from around 10 to nearly 2.
{/* TODO: add recording here */}
1. **Decentralized ID onboarding**: (.cb.id), they recently launched [base names](https://www.base.org/names) to the public. (Say hello to [renan.base.eth](https://www.base.org/names/renan.base.eth)!)
{/* TODO: add recording here */}
...
1. **A/B experiments to improve funding rates**: TODO
